...
class GigyaUserDeletionHelper implements GigyaUserDeletionHelperInterface {
  ...
  public function getFileList() {
    try {
      $secretKey      = '';
      $storageDetails = Drupal::config('gigya_user_deletion.job')
        ->get('gigya_user_deletion.storageDetails');
      $helper         = $this->helper ?? new GigyaHelper();
      ...
      Drupal::logger('gigya_user_deletion')
        ->error('Failed to get the list of files from S3 server. Error: '
                    . $e->getMessage());
      return FALSE;
    }
    catch (Exception $e) {
  ...
  public function loadFileFromServer(string $fileName) {
    ...
    try {
      ...
      Drupal::logger('gigya_user_deletion')
        ->error('Failed to get file from S3 server - ' . $e->getMessage());
      return FALSE;
    }
    catch (Exception $e) {
      Drupal::logger('gigya_user_deletion')
  ...
  public function getRegion() {
    /* Get S3 connection details from DB */
    $storedAwsParams = Drupal::config('gigya_user_deletion.job')
      ->get('gigya_user_deletion.storageDetails');
    $secretKeyEnc    = Drupal::config('gigya_user_deletion.job')
      ->get('gigya_user_deletion.storageDetails.secretKey');
    if (isset($this->helper)) {
      $helper = $this->helper;
    }
    else {
    ...
    try {
      ...
      try {
        ...
        Drupal::logger('gigya_user_deletion')
          ->error('Failed to get region from S3 server - ' . nl2br($e->getMessage()));
        return FALSE;
      }
    }
  }
